...
const JobSeekerRegisterForInterview = (): React.ReactElement => {
  ...
  const { data, refetch } = useQuery(
    ...
    () =>
      get_interviewDetail(
        typeof interview_invite_id === "string"
          ? parseInt(interview_invite_id)
          : -1
      ),
    {
  ...
  const { mutate: pick_date, isLoading: submitting } = useMutation(
    pick_interview_date,
    {
      onSuccess: () => {
    ...
    <div>
      ...
      {!data?.interview.has_picked_invitation ? (
        <div>
          ...
          <br />
          <label htmlFor="">
            View Available date for candidates to pick from:
          </label>
          <br />
          <br />
          <div style={{ display: "flex", flexWrap: "wrap", gap: "10px 6px" }}>
            {data?.interview.dates_related_data.dates.map((date, index) => (
              <div
                ...
                onClick={(e) => {
                  if (
                    selectedDate === date.available_dates ||
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "10px 6px" }}>
            {data?.interview.dates_related_data.times.map((time, index) => (
              <div
                ...
                onClick={(e) => {
                  if (
                    selectedTime === time.available_time ||
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <p>
            ...
            {data?.interview.interview_link.includes("https") ? (
              ...
              <small> {data?.interview.interview_link}</small>
            )}
          </p>
        </div>
      )}
