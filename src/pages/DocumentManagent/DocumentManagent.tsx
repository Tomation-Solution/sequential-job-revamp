...
} from "../../redux/api/documentManagement.api";
...
const DocumentManagent = (): React.ReactElement => {
  ...
  const { isLoading, data } = useQuery("docs_application_list", () =>
    getApplicationListApi({})
  );
      ...
      Cell: (tableProps: any) => (
        <div>
          <Button
            ...
            style={{
              opacity: `${
                tableProps.row.original.accept_application ? "1" : ".3"
              }`,
            }}
            onClick={(e) => {
  ...
  return (
    <div>
      ...
      <OffCanvas size={50} setIsOpen={setIsOpen} isOpen={isOpen}>
        {job_id ? <JobsRejectSlides job_id={job_id} /> : ""}
      </OffCanvas>
    </div>
  );
};
