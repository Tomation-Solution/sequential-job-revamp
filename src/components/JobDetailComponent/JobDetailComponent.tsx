...
const JobDetailComponent = (): React.ReactElement => {
  ...
  const { data: job, isLoading } = useQuery(
    ["job_detail", id],
    () => getJobDetailApi(typeof id === "string" ? id : ""),
    {
  ...
  const { mutate: apply } = useMutation(
    () => applyForJobsApi(typeof id === "string" ? id : ""),
    {
      onSuccess: (data) => {
  ...
  return (
    <JobDetailComponentContainer>
      ...
      <h3 style={{ color: "#24CDE2", padding: "1rem 0", fontWeight: 400 }}>
        Job details
      </h3>
      <MoreDetailInfo>
