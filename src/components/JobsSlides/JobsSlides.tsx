      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        {fields?.map((field, index) => (
          <JobsUploadCon for={"JobsUploadCon" + field?.name} key={field.id}>
            ...
            <input
              type="file"
              id={"JobsUploadCon" + field?.name}
              onChange={(e) => {
                if (e.target.files) {
                  console.log({ index });
                  setValue(
                    `upload_data.${index}.file`,
...
export const JobsTakeTestSlides = () => {
  return (
    <JobTakeTestContainer>
      ...
      <p>
        Congratulations , you have been scheduled for a test, please kindly take
        the test in due time in other toquicken the process
      </p>
