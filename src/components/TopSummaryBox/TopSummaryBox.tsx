...
import { selectCompanyDashboard ,changeApplicationDashboardState,CompanyDashboardSliceStateType} from "../../redux/company/companyDashboardSlice";
  ...
  return (
    <TopSummaryContainer>
      ...
      {data.map((item, index) => (
        ...
        newColor={item.color}
        onClick={e=>{
          dispatch(changeApplicationDashboardState(item.status as CompanyDashboardSliceStateType['dashboardJobSummaryStatus']))
        }}
...
export const CompanyTopSummaryBox = () => {
  ...
  const [data,setData] = useState([
    { color: "red", name: "Total Applications",status:'total_application',count:0},
    { color: "green", name: "Applicants hired" ,status:'applicants_hired' ,count:0},
    { color: "gray", name: "Total Number of Job Post",status:'total_number_of_job_post',count:0 },
    { color: "purple", name: "Closed Jobs",status:'closed_jobs', count:0},
    { color: "pink", name: "Total Number of CV",status:'total_number_of_cv' ,count:0},
    { color: "brown", name: "Active Jobs" ,status:'active_jobs',count:0},
  ])
  const dispatch = useAppDispatch()
  const { isLoading,} = useQuery('getCompanyDashboardSummaryApi',getCompanyDashboardSummaryApi,{
    'onSuccess':(data)=>{
      setData( [
        { color: "red", name: "Total Applications",status:'total_application',count:data.total_applicant},
        { color: "green", name: "Applicant Hired" ,status:'applicants_hired' ,count:data.applicant_hired},
        { color: "gray", name: "Total Number of Job Post",status:'total_number_of_job_post',count:data.total_number_of_job_post },
        { color: "purple", name: "Closed Jobs",status:'closed_jobs', count:data.closed_jobs},
        { color: "pink", name: "Total Number of CV",status:'total_number_of_cv' ,count:data.total_number_of_cv},
        { color: "brown", name: "Active Jobs" ,status:'active_jobs',count:data.active_jobs},
      ])
